...
struct candidate_map_t
{
	...
	void propagate(blt::vec2ui coords)
	{
		...
		while (!stack.empty())
		{
			...
			constexpr std::array<blt::vec2i, 4> dirs{blt::vec2i{-1, 0}, {0, -1}, {1, 0}, {0, 1}};
...
struct map_t
{
		...
		columns{columns}
	{
		...
		const blt::hashmap_t<State_t, float> weights = {
			{Start, -10},
			{Frozen, 50},
			{Land, 30},
			{Hole, 45},
		...
		// candidate_map_t WFC{rows, columns, weights, random};
