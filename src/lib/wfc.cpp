...
struct Action_t
{
    ...
    Action_t(const actions_t action) : action{action} // NOLINT
    {
    }

    operator actions_t() const // NOLINT
    {
        return action;
    }

    operator u32() const // NOLINT
    {
        return action;
    }

    constexpr static std::array<blt::vec2i, 4> to_dirs() { return {blt::vec2i{-1, 0}, {0, -1}, {1, 0}, {0, 1}}; }

    static Action_t from_dir(const blt::vec2i dir)
    {
        constexpr auto dirs = to_dirs();
        for (u32 i = 0; i < dirs.size(); i++)
        {
            if (dir == dirs[i])
                return static_cast<actions_t>(i);
        }
        return Left;
    }

    friend std::size_t hash_value(const Action_t& obj) { return obj.action; }

    friend bool operator==(const Action_t& a, const Action_t& b) { return a.action == b.action; }
...
struct candidate_map_t
{
    ...
    candidate_map_t(const u32 rows,
                    const u32 columns,
                    blt::hashmap_t<State_t, float> weights,
                    const std::vector<State_t>& starting_states,
    ...
    void allow(const State_t s1, const State_t s2, const Action_t a)
    {
        compatibility[s1][s2].insert(a);
    }

    auto& from_example(const std::vector<State_t>& data, const u32 nrows, const u32 ncols)
    {
        weights = {};
        for (u32 r = 0; r < nrows; r++)
        {
            ...
            {
                auto& state = data[c * nrows + r];
                if (!weights.contains(state))
                    weights[state] = 0;
                weights[state] += 1;

                constexpr auto dirs = Action_t::to_dirs();
                for (const auto dir : dirs)
                {
                    if (const auto new_pos = dir_valid({r, c}, dir))
                    {
                        const auto& other_state = data[new_pos->y() * nrows + new_pos->x()];
                        compatibility[state][other_state].insert(Action_t::from_dir(dir));
                    }
                }
            }
        }
    ...
    std::vector<State_t>& possible_states(const u32 r, const u32 c) { return data[r * columns + c]; }
    ...
    [[nodiscard]] blt::vec2ui find_min_entropy() const
    {
        blt::vec2ui coords;
        ...
        for (u32 r = 0; r < rows; r++)
        {
            for (u32 c = 0; c < columns; c++)
            {
                if (possible_states(r, c).size() == 1)
                    continue;
                const auto e = shannon_entropy(r, c);
                if (e < min)
                {
                    min = e;
                    coords = {r, c};
    ...
    [[nodiscard]] std::optional<blt::vec2ui> dir_valid(const blt::vec2ui pos, const blt::vec2i dir) const
    {
        const auto v = blt::vec_cast<blt::i32>(pos) + dir;
        if (v.x() < 0 || v.x() >= static_cast<blt::i32>(rows))
            return {};
        if (v.y() < 0 || v.y() >= static_cast<blt::i32>(columns))
            return {};
        return blt::vec_cast<u32>(v);
    }

    void propagate(blt::vec2ui coords)
    {
        std::stack<blt::vec2ui> stack;
        stack.push(coords);
        ...
        while (!stack.empty())
        {
            ...
            auto& cur_possible_tiles = possible_states(top.x(), top.y());

            constexpr auto dirs = Action_t::to_dirs();
            for (const auto dir : dirs)
            {
                if (const auto new_pos = dir_valid(top, dir))
                {
                    auto& adj_possible_tiles = possible_states(new_pos->x(), new_pos->y());
                    static blt::hashset_t<State_t> removes;
                    removes.clear();
                    for (auto other_tile : adj_possible_tiles)
                    {
                        bool has_any = false;
                        for (const auto cur : cur_possible_tiles)
                        {
                            if (compatibility.contains(cur))
                            {
                                if (compatibility[cur].contains(other_tile))
                                {
                                    if (compatibility[cur][other_tile].contains(Action_t::from_dir(dir)))
                                    {
                                        has_any = true;
                                        break;
                                    }
                        ...
                        if (!has_any)
                        {
                            removes.insert(other_tile);
                            stack.push(*new_pos);
                        }
                    }
                    adj_possible_tiles.erase(
                        std::remove_if(adj_possible_tiles.begin(),
                                       adj_possible_tiles.end(),
                                       [&](const auto& a) { return removes.contains(a); }),
                        adj_possible_tiles.end());
                }
            }
        }
...
struct map_t
{
    map_t(const u32 rows, const u32 columns) : number_of_states{rows * columns},
                                               rows{rows},
                                               columns{columns}
    {
        ...
        WFC.allow(Frozen, Frozen, Action_t::Left);
        WFC.allow(Frozen, Frozen, Action_t::Right);
        WFC.allow(Frozen, Frozen, Action_t::Up);
        WFC.allow(Frozen, Frozen, Action_t::Down);
        WFC.allow(Hole, Hole, Action_t::Left);
        WFC.allow(Hole, Hole, Action_t::Right);
        WFC.allow(Hole, Hole, Action_t::Up);
        WFC.allow(Hole, Hole, Action_t::Down);
        // WFC.allow(Land, Land, Action_t::Left);
        // WFC.allow(Land, Land, Action_t::Right);
        // WFC.allow(Land, Land, Action_t::Up);
        // WFC.allow(Land, Land, Action_t::Down);
        WFC.allow(Land, Frozen, Action_t::Left);
        WFC.allow(Land, Frozen, Action_t::Right);
        WFC.allow(Land, Frozen, Action_t::Up);
        WFC.allow(Land, Frozen, Action_t::Down);
        WFC.allow(Frozen, Land, Action_t::Left);
        WFC.allow(Frozen, Land, Action_t::Right);
        WFC.allow(Frozen, Land, Action_t::Up);
        WFC.allow(Frozen, Land, Action_t::Down);
        WFC.allow(Hole, Frozen, Action_t::Left);
        WFC.allow(Hole, Frozen, Action_t::Right);
        WFC.allow(Hole, Frozen, Action_t::Up);
        WFC.allow(Hole, Frozen, Action_t::Down);
        WFC.allow(Frozen, Hole, Action_t::Left);
        WFC.allow(Frozen, Hole, Action_t::Right);
        WFC.allow(Frozen, Hole, Action_t::Up);
        WFC.allow(Frozen, Hole, Action_t::Down);
        // WFC.allow(Hole, Land, Action_t::Right);
        // WFC.allow(Land, Hole, Action_t::Up);
        // WFC.allow(Land, Hole, Action_t::Down);
        // WFC.allow(Hole, Land, Action_t::Left);

        while (!WFC.is_complete())
        {
            auto min_entro = WFC.find_min_entropy();
            WFC.collapse(min_entro.x(), min_entro.y());
            WFC.propagate(min_entro);
        }
        data = WFC.generate_states();
    }
    ...
    [[nodiscard]] size_t get_number_of_states() const { return number_of_states; }

    [[nodiscard]] u32 get_rows() const { return rows; }

    [[nodiscard]] u32 get_columns() const { return columns; }

    StateData_t& get_state(const u32 state) { return data[state]; }

    [[nodiscard]] const StateData_t& get_state(const u32 state) const { return data[state]; }

    StateData_t& get_state(const u32 r, const u32 c) { return data[r * columns + c]; }

    [[nodiscard]] const StateData_t& get_state(const u32 r, const u32 c) const { return data[r * columns + c]; }

    StateData_t& get_state(const blt::vec2ui& pos) { return data[pos.x() * columns + pos.y()]; }
    ...
    blt::random::random_t& get_random() { return random; }
...
static void init(const blt::gfx::window_data&)
{
    ...
    resources.enqueue("res/frozen.png", "frozen");
    resources.enqueue("res/goal.png", "goal");
    resources.enqueue("res/hole.png", "hole");
    resources.enqueue("res/land.png", "land");
    resources.enqueue("res/start.png", "start");
...
static void update(const blt::gfx::window_data& data)
{
    ...
    for (u32 i = 0; i < 4; i++)
    {
        for (u32 j = 0; j < 4; j++)
        {
            ...
            renderer_2d.drawRectangle({map.get_pos(i, j), map.get_size()}, texture);
        }
    }
